...
router = APIRouter(prefix='/user_authentication', tags=['Аутенфикация пациентов'])
...
async def login(
    ...
    user = await UserAuthMethods.validate_object(form_data.username, form_data.password)
    if not user:
        raise HTTPException(
